...
const { data: post } = await useAsyncData(`post-${postId}`, () =>
  api(`posts/${postId}`),
);
...
<template>
  <div class="app-container">
     ...
     <div class="post-page post">
      <h1>{{ post.title }}</h1>
        <div class="post-content big">
          <p style="line-clamp: 10; -webkit-line-clamp: 10">{{ post.content }}</p>
        </div>
        <div class="back-link-div">
          <NuxtLink to="/posts" class="back-link">- К списку постов</NuxtLink>
          <NuxtLink :to="`/redact/${post.id}`" class="back-link">Отредакировать пост -</NuxtLink>
        </div>
      </div>
      <div>
    ...
    <div class="form-group">
      <label for="post-title" class="input-label">Ваше имя</label>
      <input
        id="post-title"
